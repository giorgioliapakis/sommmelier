...
def analyze_confidence_intervals(results: dict[str, Any]) -> list[ImprovementQuestion]:
...
def analyze_model_fit(results: dict[str, Any]) -> list[ImprovementQuestion]:
...
def analyze_channel_structure(results: dict[str, Any]) -> list[ImprovementQuestion]:
...
def analyze_data_completeness(
    results: dict[str, Any], has_impressions: bool = False
) -> list[ImprovementQuestion]:
    ...
    if not has_impressions:
        questions.append(
            ImprovementQuestion(
                ...
                why_it_helps="The run used the explicitly enabled $10 CPM fallback for at least one channel. "
                "Actual impressions avoid imposing a fabricated execution-to-spend relationship.",
...
def analyze_time_dynamics(results: dict[str, Any]) -> list[ImprovementQuestion]:
...
def analyze_spend_concentration(results: dict[str, Any]) -> list[ImprovementQuestion]:
    """Flag channels that dominate the budget, making other channels hard to measure."""
    questions: list[ImprovementQuestion] = []
    metadata = results.get("metadata", {})
...
def analyze_brand_search_inflation(results: dict[str, Any]) -> list[ImprovementQuestion]:
...
def analyze_organic_baseline(results: dict[str, Any]) -> list[ImprovementQuestion]:
    ...
    media_total = sum(
        c.get("absolute", 0) if isinstance(c, dict) else c for c in contributions.values()
    )
...
def analyze_adstock_plausibility(results: dict[str, Any]) -> list[ImprovementQuestion]:
...
def analyze_geographic_signal(results: dict[str, Any]) -> list[ImprovementQuestion]:
...
def analyze_prior_posterior_divergence(results: dict[str, Any]) -> list[ImprovementQuestion]:
...
def generate_improvement_questions(
    results: dict[str, Any], has_impressions: bool | None = None
) -> list[ImprovementQuestion]:
    """
    ...
    """
    if has_impressions is None:
        has_impressions = not bool(
            results.get("metadata", {}).get("estimated_impression_channels", [])
        )
...
def format_questions_as_checklist(questions: list[ImprovementQuestion]) -> str:
    ...
    by_category: dict[str, list[ImprovementQuestion]] = {}
    for q in questions:
